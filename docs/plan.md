# Implementation Plan: YESIR（TriLayer + Inquire）

> Design: `docs/design-trilayer.md`（已确认）｜Spec: `docs/spec.md`

## Component Map

NEW: `pyproject.toml`, `scripts/check.ps1`, `yesir/*`（9 模块）, `web/{index.html,app.js,style.css}`, `tests/{test_trilayer,test_session,test_tools}.py`
...
**Files:** `pyproject.toml`, `yesir/__init__.py`, `yesir/__main__.py`（占位 arg 解析）, `scripts/check.ps1`
**Acceptance:** `ruff check .` 与 `ruff format --check .` 通过；`python -m yesir --help` 打印用法。
**Depends:** none

### Task 2: config + session（兼容旧格式）
**What:** Config 加载（config.json > env）；会话列表/读写删（沿用旧 JSON schema）。
**Files:** `yesir/config.py`, `yesir/session.py`, `tests/test_session.py`
...
**Files:** `yesir/tools/{__init__,files,shell,search,webtools}.py`, `tests/test_tools.py`
...
**Files:** `yesir/events.py`, `yesir/agent.py`
**Acceptance:** FakeLLM 驱动一轮"调用工具→回填→收尾"的循环验证；`python -m yesir "你好"` 控制台单发真实 API 可用。
**Depends:** 3, 4

### Task 6: Web server（原功能全量）
**What:** ThreadingHTTPServer + 全部旧端点 + 静态文件；`/chat` NDJSON（WebSink 加锁）。
**Files:** `yesir/server.py`
...
**Files:** `yesir/trilayer.py`, `yesir/agent.py`（spawn 工具接线）, `tests/test_trilayer.py`
...
**Files:** `yesir/tools/ask.py`, `yesir/server.py`, `web/{app.js,index.html,style.css}`, `yesir/agent.py`
